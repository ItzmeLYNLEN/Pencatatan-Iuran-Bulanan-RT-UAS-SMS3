...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['change_password'])) {
    ...
    $stmt = $conn->prepare("SELECT password FROM warga WHERE id_warga = ?");
    ...
    if ($user && $current_password === $user['password']) {
        if ($new_password === $confirm_password) {
            if (strlen($new_password) >= 6) {
                ...
                $update_stmt->bind_param("si", $new_password, $id_warga);
...
<div class="profile-wrapper">
    <div class="profile-card">
        <div class="profile-card-header">
            <h4><i class="fas fa-user-shield me-2"></i>Ubah Password</h4>
        </div>
        <div class="profile-card-body">
            <form method="POST" action="">
                ...
                <div class="d-grid gap-2 mt-4">
                    <button type="submit" name="change_password" class="btn btn-primary btn-lg">Simpan Perubahan</button>
